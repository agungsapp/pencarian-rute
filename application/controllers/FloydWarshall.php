...
class FloydWarshall
{
	...
	public function __construct($graf = '', $grafName = '', $lok_asal = '', $lok_akhir = '', $times = '')
	{
		...
		for ($i = 0; $i < count($this->graf); $i++) {
			for ($j = 0; $j < count($this->graf); $j++) {
				for ($k = 0; $k < count($this->graf); $k++) {
					$this->graf[$i][$j] = min($this->graf[$i][$j], $this->graf[$i][$k] + $this->graf[$k][$j]);
				}
			}
		}
	}
...
// Kode di atas merupakan implementasi dari algoritma Floyd-Warshall untuk mencari jarak terpendek dalam sebuah graf. Algoritma ini menggunakan teknik dynamic programming untuk menemukan jarak terpendek antara setiap pasangan node dalam graf.

// Pada bagian constructor, kode di atas mengambil input sebuah graf, nama-nama dari node dalam graf, node awal dan akhir, serta waktu yang ditentukan. Kemudian, kode mengambil setiap pasangan node dalam graf dan menghitung jarak terpendek antara keduanya menggunakan perulangan for yang di dalamnya terdapat perulangan for lainnya.

// Algoritma Floyd-Warshall mengevaluasi jarak dari setiap pasangan node dengan mengevaluasi jarak dari setiap node lain yang berada di antara kedua node tersebut. Jika jarak dari node lain lebih pendek dari jarak yang sebelumnya ditentukan, maka jarak tersebut diperbarui.

// Pada akhirnya, graf yang diinput akan diupdate dengan jarak terpendek antara setiap pasangan node yang diperoleh dari algoritma.

// Namun, perhatikan bahwa kode di atas hanya merupakan contoh sederhana dan mungkin tidak bekerja sesuai dengan konteks khusus Anda, karena tidak lengkap.
